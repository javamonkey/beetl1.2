...
public class LayoutFunctionTest extends BasicTestCase {
	...
	public void testLayout() throws IOException, BeeException {
		...
		try {
			Template t = this.gt
					.getFileTemplate("/ext/layout_child_template.html");
			t.set("name", "joel");
			t.set("b", 1);
			String str = t.getTextAsString();

			// Assert.assertEquals(this.getFileContent("/ext/layout_expected.html"),
			// str);
			// 第二次预编译
	...
	// {
	// Template t =
	// this.gt.getFileTemplate("/ext/complex_child_layout_template.html");
	//
	// String str = t.getTextAsString();
	//
	// Assert.assertEquals(this.getFileContent("/ext/complex_layout_expected.html"),
	// str);
	// // 第二次预编译
