...
public class DirectiveSafeOutputTestCase extends BasicTestCase {
	...
	public void testBasic() throws IOException, BeeException {
		...
		Template t = this.gt
				.getFileTemplate("/directive/safe_output_template.html");
		User user = new User();
		t.set("user", user);
		String str = t.getTextAsString();
		this.assertEquals(
				this.getFileContent("/directive/safe_output_expected.html"),
				str);
	 ...
	 * 
	 * Template t =
	 * this.gt.getFileTemplate("/directive/safe_output2_template.html"); User
	 * user = new User(); t.set("user", user); String str = t.getTextAsString();
	 * this
	 * .assertEquals(this.getFileContent("/directive/safe_output2_expected.html"
	 * ), str);
	 * 
	 * t = this.gt.getFileTemplate("/directive/safe_output2_template.html");
	 * t.set("user", user); str = t.getTextAsString();
	 * this.assertEquals(this.getFileContent
	 * ("/directive/safe_output2_expected.html"), str);
	 * 
	 * }
	 */
