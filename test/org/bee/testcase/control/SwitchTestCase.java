...
public class SwitchTestCase extends BasicTestCase {
	...
	public void testSimple() throws IOException, BeeException {
		...
		Assert.assertEquals(
				this.getFileContent("/control/switch_expected.html"), str);
		// 第二次预编译
	...
	public void testSimple1() throws IOException, BeeException {
		...
		String str = t.getTextAsString();
		Assert.assertEquals(
				this.getFileContent("/control/switch1_expected.html"), str);
		// 第二次预编译
	...
	public void testSimple2() throws IOException, BeeException {
		...
		Template t = this.gt
				.getFileTemplate("/control/switch_default_template.html");
		User user = new User();
		user.setAge(55);
		user.setName("joel");
		t.set("user", user);
		...
		Assert.assertEquals(
				this.getFileContent("/control/switch_default_expected.html"),
				str);
	}
