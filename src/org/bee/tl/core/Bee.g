...
//ASTLabelType=CommonNodeTree;
ASTLabelType=BeeCommonNodeTree;
...
tokens {
  VAR_DEFINE;   // variable definition
  VAR_REFER;
  ASSIGNMENT;
  HOLDER;
  TEXT_HOLDER;
...
messFunction options{
backtrack=false;
}
	:	{isTextProcessFunction(input)}? textProcessFunction
	|   functionCall[true] ';';

textProcessFunction
	
	:	functionFullName a='(' (exp (',' exp)*)? ')' statmentBlock -> ^(TEXT_PROCESS[$a] functionFullName statmentBlock (exp)*);
	
		
functionCall [boolean statmentCall]
	@after{
		if(isStrictMVC){
			throw new MVCStrictException($start);			
		}
	}
	:functionFullName a='(' (exp (',' exp)*)? ')' 
		-> {statmentCall}? ^(DIRECT_CALL ^(FUNCTION[$a] functionFullName exp*))
...
forStatment
	:	'for' '(' a=Identifier 'in ' aaa= exp ')'    c=statmentBlock  ('elsefor' d=statmentBlock )?
	 -> ^(FOR $a $aaa  $c $d? ); 
whileStament
	:	'while' '(' a=condExp ')'  statmentBlock -> ^(WHILE $a statmentBlock) ;

ifStatment
	:	'if' '(' condExp ')' l=statement ('else' n=statement )?  ->^(IF condExp $l $n? ) ;
	
constantsTextStatment
	:	LEFT_TEXT_TOKEN  Identifier RIGHT_TOKEN   ->^(TEXT_HOLDER Identifier ) ;
...
varAssignMent	
	...
	|	Identifier '=' statmentBlock  ->   ^(ASSIGNMENT[$Identifier] Identifier statmentBlock) ;

assignMent
	:	Identifier '='  exp  -> ^(ASSIGNMENT[$Identifier] Identifier exp) ;

varRef	:	{input.LT(1).getText().startsWith("__V")}? Identifier   -> ^(TEXT_VAR_REFER[$Identifier] Identifier )
	|   Identifier{if($Identifier.getText().startsWith("__"))throw new RuntimeException("Can Not call this Variable star with __"); } ( varAttribute)* safeOutput?   -> ^(VAR_REFER[$Identifier] Identifier (varAttribute)* safeOutput? )
...
varAttribute
	:	'.' Identifier -> ^(ATTR_NAME  Identifier)
	|	'.~' Identifier  -> ^(VIRTUAL_ATTR_NAME  Identifier)
	|	a='[' exp ']' ->^(MAP_LIST_INDEX[$a,"[]"] exp)
...
conditionalAndExpression 
    ...
    ;
condExpNotItem 
	:	condExpItem ->condExpItem
	|	('!') condExpItem -> ^(NOT condExpItem );
condExpItem
	:	  aexpr ( (a=EQUAL^|a=NOT_EQUAL^|a=LARGE^|a=LARGE_EQUAL^|a=LESS^|a=LESS_EQUAL^){if(isStrictMVC)	throw new MVCStrictException($a);} l=aexpr )?;
aexpr	:	mexpr ((a=ADD^|a=MINUS^) mexpr {if(isStrictMVC)	throw new MVCStrictException($a);})* ;
mexpr   :       unaryAtom ((a=MULTIP^|a=DIV^|a=MOD^){if(isStrictMVC)	throw new MVCStrictException($a);} unaryAtom)*;
atom	:	BOOLEAN
	
	|   INT
	|	NULL
	|	DOUBLE
	|	a=StringLiteral -> {new BeeCommonNodeTree(new CommonToken(StringLiteral,BeetlUtil.getEscapeString($a.text)))}
	|	varRef 
	|	'('! exp ')'! 
	|	functionCall[false]
	|	nativeMethod[false]
